...
class Builder(Tk):
    def __init__(self: object) -> None:
        ...
        self.user_desktop: str = join(expanduser("~"), "Desktop")
                # file to minify
        self.files_to_minify: tuple = ('Sounder5.py', 'Updater.py', 'Components\\Setup.py', 'Components\\SongMenu.py', 'Components\\SystemTheme.py', 'Components\\DirWatcher.py', 'Components\\FontManager.py')
        # updater setup
        self.build_updater: list = ['pyinstaller', '-w', '--onefile', '--uac-admin', '-i', abspath('Resources\\Icons\\Updater\\setup.ico'), 'Updater.py']
        # sounder setup
        self.build_sounder: list = ['pyinstaller', '-w', '-i', abspath('Resources\\Icons\\Light\\icon.ico'), 'Sounder5.py']
    ...
    def __init_ui__(self: object) -> None:
        ...
        # init console
        self.console: Text = Text(left_frame, state='disabled', background='#212121', foreground='#fff',selectbackground="#ecf0f1", selectforeground="#212121", bd=0, cursor="arrow", takefocus=0, font=('Catamaran bold', 10), yscrollcommand=scrollbar.set)
        # configure tags
        self.console.tag_config('err', background='#212121', foreground='#c0392b')
        self.console.tag_config('info', background='#212121', foreground='#ecf0f1')
        self.console.tag_config('warning', background='#212121', foreground='#f1c40f')
        self.console.tag_config('embeded', background='#212121', foreground='#2980b9')
        self.console.tag_config('success', background='#212121', foreground='#27ae60')
        ...
        self.build_button: ttk.Button = ttk.Button(buttons_frame, text='Build', takefocus=False, command=self.build)
        self.build_button.pack(side='left', padx=5)
        ttk.Button(buttons_frame, text='Follow output', takefocus=False, command=self.change_follow).pack(side='left', padx=5)
        buttons_frame.place(relx=0.95, rely=0.95, anchor='se')

    def change_follow(self: object) -> None:
        self.follow = not self.follow
        self.__push_msg_(f'Follow console output: {"Enabled" if self.follow else "Disabled"}', type='warning')
    ...
    def build_process(self: object) -> None:
        ...
        self.__push_msg_(f'Using shell commands {self.build_updater}', type='warning')
        ...
        if exists('build'):
            ...
            sleep(2)
        self.__push_msg_('Preparing console', type='warning')
        self.__push_msg_(f'Using shell commands {self.build_sounder}', type='warning')
        ...
        if not self.__copy_folders():
            self.__push_msg_('Stopping due to encounter error', type='err')
            return None
        self.__push_msg_('Succesfouly copied: Resourcess, Components', type='success')
        ...
        self.__push_msg_(f'Task completed successfully in {end - start:0.4f} seconds', type='success')
        self.build_button.state(['!disabled'])
    ...
    def __copy_updater(self: object) -> bool:
        try:
            if exists('Sounder files'):
                move(join(self.cwd, 'Build', 'dist', 'Updater.exe'), join(self.user_desktop, 'Sounder files'))
                return True
            return False
        except Exception as err_obj:
            self.__push_msg_(f'Error: {err_obj}', type='err')
    ...
    def __copy_sounder(self: object) -> bool:
        try:
            if exists('Sounder files'):
                copytree(join(self.cwd, 'Build', 'dist', 'Sounder5'), 'Sounder files', dirs_exist_ok=True)
                return True
            return False
        except Exception as err_obj:
            self.__push_msg_(f'Error: {err_obj}', type='err')
    ...
    def __copy_folders(self: object) -> bool:
        try:
            ...
            remove(join('Sounder files', 'Resources', 'Icons', 'Updater', 'cat.gif'))
            remove(join('Sounder files', 'Resources', 'Icons', 'Updater', 'setup.ico'))
